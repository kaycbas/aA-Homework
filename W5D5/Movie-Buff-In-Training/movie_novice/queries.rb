...
def below_average_years
  ...
  Movie.select('count(*) AS bad_movies, yr').where('score < 5').group(:yr).order('bad_movies DESC')
end
...
def pulp_fiction_actors
  ...
  Movie.select('actors.id, actors.name').joins(:actors).where("title = 'Pulp Fiction'")
end
